...
export function useMissionContextMenu() {
  ...
  const handleContextMenu = (e, waypointId = null) => {
    ...
    const detectedWaypointId = idEl?.dataset?.waypointId ?? idEl?.dataset?.id ?? null
    setClickedWaypointId(detectedWaypointId ?? waypointId ?? null)
    console.log('Right-click waypoint id:', detectedWaypointId ?? waypointId ?? null)
    // Save coordinates for future use
    setSavedCoordinates(prev => [...prev, {
      lat: e.lngLat.lat,
      lng: e.lngLat.lng,
      timestamp: new Date().toISOString(),
      waypointId: detectedWaypointId ?? waypointId ?? null
    }])
...
export default function MissionContextMenuOverlay({ ctx }) {
  ...
  return (
    <div
      ...
      style={{ 
        top: ctx.points?.y ?? -9999, 
        left: ctx.points?.x ?? -9999,
