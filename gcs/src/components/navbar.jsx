...
import { RunCommand } from "./spotlight/commandHandler.js"
...
function NavbarContent({ currentPage }) {
  const {
    connected,
    connectedToSocket,
    ConnectionType,
  ...
  return (
    <div className="w-full flex justify-between gap-x-4 xl:grid xl:grid-cols-2 xl:gap-0">
        ...
        <div className="!ml-auto flex flex-row space-x-4 items-center">
          ...
          {connectedToSocket ? (
            <Button
              onClick={connected ? () => RunCommand("disconnect_from_drone") : () => RunCommand("connect_to_drone")}
              color={
                connected
